...
namespace GestPharmaDB_Parametre
    ...
    class Program
    {
        ...
        const string CONNECTION_STRING2 = @"Data Source=LENOVO;Initial Catalog=BDPM;Integrated Security=True";
        ...
        static void Main(string[] args)
        {
            ...
            for (int nbtable = 0; nbtable < tables.Length; nbtable++)
                ...
                transaction = connection.BeginTransaction($"CIS Transaction {nbtable}");
            ...
            Console.WriteLine($"\n\n Début déploiement BDPM ==> {DateTime.Now}");
            Console.WriteLine($"\n\n       Traiement en cours....");
            try
                {

                var dacPackageLocation = Path.Combine(@"C:\Users\phili\source\repos\GestPharmaDB\GestPharmaDB\bin\Debug\", "GestPharmaDB.dacpac");

                var dacPackage = DacPackage.Load(dacPackageLocation);
                var dacServices = new DacServices(CONNECTION_STRING2);
            ...
            Console.WriteLine($"\n\n Fin déploiement BDPM ==> {DateTime.Now}");
            }
    }
}
